...
Like VMs, Containers are fundamental for modern software industry. Provides a safe and controlled environment to run your code, with no hardware virtualization and high performance, similar to the native one. \\
The use cases are analogous to and have the same versatility as VMs. \\
...
To create the cluster in Docker is virtually mandatory to use the feature \texttt{docker compose}. \\
...
The last thing --- for now --- to implement is the resource limits. In the VM is mandatory writing the maximum resources, in docker is not. \\
...
\begin{itemize}
    \item there are some commands that allow to use SSH from master to node, this because linux denies access to the root user by default;
...
\begin{itemize}
    \item the process is not automated
